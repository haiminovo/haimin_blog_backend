 ...
 * 喜欢文章
 */
router.put('/article/like/:id', async (ctx) => {
  // 通过验证器校验参数是否通过
  const v = await new PositiveIdParamsValidator().validate(ctx);
  ...
  const [err, data] = await ArticleDao.detail(id, ctx.query);
    if (!err) {
        // 更新文章浏览
        await ArticleDao.updateFavorite(id, ++data.favorite_num);
        // 返回结果
        ctx.response.status = 200;
...
router.get('/article/find', async (ctx) => {
  // 尝试获文章取缓存
  const { admin_id,category_id = 0, page = 1 } = ctx.query;

  // 没有缓存，则读取数据库
  const [err, data] = await ArticleDao.find(ctx.query);
  if (!err) {
      ctx.response.status = 200;
      ctx.body = res.json(data);
  } else {
...
router.get('/article/:id', async (ctx) => {
    ...
    if (!err) {
        ...
        if (!commentError) {
            data.dataValues.comment = commentData.data;
        }
